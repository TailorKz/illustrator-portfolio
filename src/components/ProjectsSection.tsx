...
function PillButton({ onClick, label, className = "" }: { onClick: () => void; label: string; className?: string; }) {
  ...
  return (
    <button onClick={onClick} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} className={`group relative inline-flex cursor-pointer items-center gap-2 px-8 py-3 font-sans text-sm font-bold uppercase tracking-widest text-ink transition-colors duration-300 group-hover:text-pink ${className}`}>
      ...
      <motion.svg className="relative z-10 h-4 w-4" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24" animate={{ x: hovered ? 4 : [0, 3, 0] }} transition={hovered ? { duration: 0.3, ease: "easeOut" } : { duration: 1.4, repeat: Infinity, ease: "easeInOut" }}>
        <path strokeLinecap="round" strokeLinejoin="round" d="M14 5l7 7m0 0l-7 7m7-7H3" />
      </motion.svg>
      <svg className="pointer-events-none absolute inset-0 h-full w-full overflow-visible" viewBox="0 0 200 70" preserveAspectRatio="none">
        <motion.path d="M15,35 C10,12 45,4 100,5 C160,6 195,15 190,38 C185,63 145,68 100,66 C50,64 20,60 15,35Z" fill="none" stroke="var(--color-pink)" strokeWidth="3" strokeLinecap="round" initial={false} animate={{ pathLength: hovered ? 1 : 0, opacity: hovered ? 1 : 0 }} transition={{ duration: 0.5, ease: "easeInOut" }} />
      </svg>
    </button>
  );
}
  ...
  return (
    <motion.button onClick={onClick} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} whileHover={{ y: -3 }} whileTap={{ scale: 0.96 }} className="group flex items-center gap-4 cursor-pointer">
      ...
      <span className="flex h-12 w-12 items-center justify-center rounded-full border-2 border-pink-soft bg-white transition-colors duration-300 group-hover:border-pink group-hover:bg-pink-soft/40">
        <motion.svg className="h-5 w-5 text-pink" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24" animate={{ x: hovered ? 5 : [0, 3, 0] }} transition={hovered ? { duration: 0.3, ease: "easeOut" } : { duration: 1.6, repeat: Infinity, ease: "easeInOut" }}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M14 5l7 7m0 0l-7 7m7-7H3" />
        </motion.svg>
      </span>
    </motion.button>
...
export function ProjectsSection({ onSeeAll }: { onSeeAll: () => void }) {
  ...
  const tags = useMemo(() => ["Principais", ...Array.from(new Set(projects.flatMap((p) => p.tags)))], []);

  const filtered = useMemo(() => {
    const list = activeTag === "Principais" ? projects : projects.filter((p) => p.tags.includes(activeTag));
